...
export async function sendEmailAlert(email: string, message: string): Promise<boolean> {
  try {
    ...
    await emailTransporter.sendMail({
      ...
      to: email,
      subject: '🚨 Sakhi Suraksha - Emergency Alert',
      html: `
...
export async function sendEmailOTP(email: string, otp: string): Promise<boolean> {
  try {
    ...
    await emailTransporter.sendMail({
      from: 'noreply@sakhi-suraksha.app',
      to: email,
      subject: 'Sakhi Suraksha - Verification Code',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      `,
      text: `Your Sakhi Suraksha verification code is: ${otp}. This code will expire in 10 minutes.`
    });
